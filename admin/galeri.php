...
<head>
    ...
    <title>Galeri | Warung Pandansari</title>
            ...
            <div class="box">
                ...
                <div class="box-body table-responsive no-padding">
                    <table class="table table-hover">
                        ...
                        <?
                        $stmt = $info->fetchGaleri();
                        $stmt->bind_result($id, $gambar);
                        while ($stmt->fetch()) {
                            ?>
                            <tr>
                                <td><? echo $id ?></td>
                                <td><img src="../images/galeri/<? echo $gambar ?>" class="image img-bordered-sm" width="60" height="50"></td>
                                <td>
                                    <input type="hidden" id="galeri-id" value="<? echo $id?>">
                                    <input type="hidden" id="old-image" value="<? echo $gambar?>">
                                    <a href="#" class="btn btn-danger hapus delete" onclick="delete_galeri('<? echo $id?>', '<? echo $gambar?>')" ><i class="fa fa-trash"></i> </a>
                                </td>
                            </tr>
                            <?
                        }
...
<script>
  function delete_galeri(id, image_old){
    var inputan = '{ "id" : "'+id+'", "old": "'+image_old+'" }';
    $.ajax({
        url: 'api/info.php?action=delete-galeri',
        type: 'PUT',
        data: inputan,
        dataType: 'json',
    ...
    $(document).ready(function () {
        ...
        $('#simpan').click(function () {
            var inputan = '{ "gambar": "' + gambar + '"}';
            $.ajax({
                url: 'api/info.php?action=add-galeri',
                type: 'PUT',
                data: inputan,
                dataType: 'json',
                success: function (result) {
